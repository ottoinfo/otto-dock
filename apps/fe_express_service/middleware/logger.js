...
const { APP_ENV, NODE_ENV, KUBERNETES_SERVICE_HOST } = process.env;
...
const logger = createLogger({
  defaultMeta: {
    app_env: APP_ENV,
    node_env: NODE_ENV,
    service: "easel ( NODE-JS )",
    timestamp: new Date().toLocaleString()
  }, // TODO - before more specific on USER, ENV, BRANCH
  exitOnError: false, // do not exit on handled exceptions
  level,
  format: format.json(),
  transports: [new transports.Console()]

  // transports: KUBERNETES_SERVICE_HOST
  //   ? [new Elasticsearch(options.elasticsearch)]
  //   : [
  //     // new winston.transports.File(options.file),
  //     new transports.Console(options.console),
  //   ],
  // ...(dev ? { format: combine(prettyPrint()) } : null),
});
